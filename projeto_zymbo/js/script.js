...
document.querySelector(".verTudo").addEventListener("click", function() {
  ...
  let visivel = Array.from(ocultos).some(card => card.style.visibility === "visible");

  ocultos.forEach(card => {
    card.style.visibility = visivel ? "hidden" : "visible";
    card.style.position = visivel ? "absolute" : "relative";
  });
